...
const canWeDeliver = (zipCode, arr) => {
    return arr.includes(zipCode) 
    ? `You're in our delivery zone!`
    : `Sorry, we can't deliver to that address`;
}

console.log(canWeDeliver(85209, deliveryAreaZipCodes))
...
// CODE HERE
const canWeDeliverTwo = (zipCode, zipCodes) => {
    for(const zip of zipCodes) {
        if(zip === zipCode) return `You're in our delivery zone!`;
        else return `Sorry, we can't deliver to that address`;
    }
}

onsole.log(canWeDeliverTwo(85209, deliveryAreaZipCodes))
cconsole.log(canWeDeliverTwo(85205, deliveryAreaZipCodes))
...
deal down to 10%.
   ...
   //CODE HERE
   const tenPercent = deals.map(deal => deal.title.replace('15', '10'))
  ...
  //CODE HERE
  const month = deals.map(deal => deal.desc.replace('March', 'April'))
  console.log(month)
